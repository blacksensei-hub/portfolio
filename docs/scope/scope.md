...
| 13 | Visitor analytics | Release 3 | done |
...
### 10. Deploy to free hosting · done
...
- [x] Build it: `/develop deploy to free hosting`
  - [x] GitHub repo, Cloudflare Pages project, token, and secrets (AC-1, AC-2)
  - [x] Real `SITE_URL` and the `check` job with concurrency (AC-3, AC-5, AC-7)
  - [x] `deploy` job with the fork guard, production and previews (AC-2, AC-4, AC-6)
  - [x] First live deploy and a test PR (AC-1 to AC-7)
- [x] Verify it: `/check verify deploy to free hosting`
...
- [x] Design it (spec): `/architect dark/light theme`
- [x] Build it: `/develop dark/light theme`
...
- [x] Verify it: `/check verify work with me section`

## Release 3: learn who visits

### 13. Visitor analytics · done
...
- [x] Build it: `/develop visitor analytics`
  ...
  - [x] E2E for the tag vs env, blocked requests, cookies, footer and axe, then a live check (AC-1 to AC-5)
- [x] Verify it: `/check verify visitor analytics`
